-- ══════════════════════════════════════════════════════════
-- SM-Group — Migration V26 — إصلاح: "DELETE requires a WHERE clause"
--
-- السبب الجذري:
--   قاعدة بيانات Supabase الخاصة بك تستخدم إضافة أمان على
--   مستوى Postgres (تشبه safeupdate) ترفض أي DELETE أو UPDATE
--   بدون WHERE — حتى لو كان الاستدعاء من داخل دالة RPC محمية
--   بصلاحيات SECURITY DEFINER. دالة admin_restore_backup في
--   MIGRATION_V23 كانت تحذف الجداول بأوامر DELETE مجرّدة بلا
--   WHERE (لأن الهدف كان مسح الجدول بالكامل)، فرفضتها القاعدة.
--
-- الإصلاح: إضافة WHERE true لكل عملية حذف — تحذف كل الصفوف
-- فعلياً (الشرط دائماً صحيح) لكنها تحقق الشرط التركيبي الذي
-- تفرضه القاعدة. لا تغيير في السلوك الفعلي، فقط في الصياغة.
--
-- ⚠️ هذا الملف يستبدل الدالة بالكامل (CREATE OR REPLACE) —
-- لا يُعدَّل MIGRATION_V23 نفسه أبداً (سجل تاريخي غير قابل للتعديل).
-- ══════════════════════════════════════════════════════════
