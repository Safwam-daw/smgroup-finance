...
BEGIN
  ...
  -- يدعم الهاش القديم (SHA-256 hex، طوله 64) والجديد (bcrypt عبر extensions.crypt())
  IF length(u.pass) = 64 THEN
    ok := u.pass = encode(extensions.digest(p_caller_password, 'sha256'), 'hex');
  ELSE
    ok := u.pass = extensions.crypt(p_caller_password, u.pass);
  END IF;
...
BEGIN
  ...
  IF length(u.pass) = 64 THEN
    ok := u.pass = encode(extensions.digest(p_current_pass, 'sha256'), 'hex');
  ELSE
    ok := u.pass = extensions.crypt(p_current_pass, u.pass);
  END IF;
  ...
  UPDATE public.users
  SET pass = extensions.crypt(p_new_pass, extensions.gen_salt('bf', 10))
  WHERE id = u.id;
...
-- VALUES (
--   'admin',
--   extensions.crypt('CHANGE_ME_123', extensions.gen_salt('bf', 10)),
--   'admin',
--   NULL
-- );
-- ══════════════════════════════════════════════════════════
